...
def find_hough(img, as_pandas=False, dp=1.2, minDist=10, param1=50, param2=15, minRadius=3, maxRadius=20):
    ...
    if circles is not None:
        ...
        for i in circles[0, :]:
            x, y = i[0], i[1]
            results.append((x, y, 0, 0))  # Dummy RA/DEC for now

    return results, results

# def find_stars(img, as_pandas=False, method='hough'):
#     h, w = img.shape[:2]
#     small_image = h < 500 or w < 500
#
#     # Adjust sensitivity parameters
#     if method == 'blob':
#         min_area = 3 if small_image else 8
#         threshold = 10 if small_image else 20
#         return __find_blobs(img, as_pandas)
#     else:  # method == 'hough'
#         min_dist = 5 if small_image else 10
#         param1 = 30 if small_image else 50
#         param2 = 5 if small_image else 15
#         min_radius = 3 if small_image else 10
#         max_radius = 7 if small_image else 12
#         return find_hough(img, as_pandas,
#                             dp=1.2,
#                             minDist=min_dist,
#                             param1=param1,
#                             param2=param2,
...
def annotate_stars(fits_path, stars, output_txt):
    ...
    try:
        ...
        custom_simbad.add_votable_fields('ids')  # To get extra catalog names
        ...
        for x, y, *_ in stars:
            ...
            if simbad_result:
                main_id = simbad_result[0]['MAIN_ID'].decode('utf-8') if isinstance(simbad_result[0]['MAIN_ID'], bytes) else simbad_result[0]['MAIN_ID']
                catalog_ids = simbad_result[0]['IDS']
                name = f"{main_id} ({catalog_ids})"

                # Skip if this object was already seen
                if main_id in seen_names:
                    continue
                seen_names.add(main_id)
            else:
                # If unknown, skip annotation entirely
                continue
...
def show_detected_stars_with_names(image_path, annotated_stars):
    ...
    window_name = 'Annotated Stars with Names'

    # Draw only known stars (with names)
    for (x, y, ra, dec, name) in annotated_stars:
        if not name.strip():
            continue
        x, y = int(x), int(y)
        cv2.circle(annotated_img, (x, y), 10, (0, 255, 0), 2)
        cv2.putText(annotated_img, name, (x + 15, y), cv2.FONT_HERSHEY_SIMPLEX,
    ...
    while True:
        display_img = annotated_img if show_annotations else original_img
...
if __name__ == "__main__":
  ...
  #  annotate_stars(FITS_OUTPUT, stars, TXT_OUTPUT)
    ...
    image_path = r"C:\Users\alons\Downloads\project_new_space-master\project_new_space-master\photo\tycho.jpg"  # Example: "images/stars_sample.jpg"
